    ...
    <>
      {title && (
        <header className="mb-10">
          ...
          {subtitle && (
            <p className="mt-4 text-lg md:text-xl text-[var(--foreground-muted)] font-medium max-w-3xl">
              {subtitle}
            </p>
          )}
        </header>
  ...
  return (
    <section
      ...
      className={cn(
        ...
        fullWidth && (alt ? "bg-[var(--surface)]" : "bg-[var(--background)]"),
        !fullWidth && "mx-auto max-w-7xl px-5 md:px-6",
        className
      )}
    >
      {fullWidth ? <div className="mx-auto max-w-7xl px-5 md:px-6 lg:px-8">{inner}</div> : inner}
    </section>
  );
}
